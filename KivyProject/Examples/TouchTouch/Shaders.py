...
class ShaderScatter(Scatter):
    def __init__(self, source=None, texture=None, repeat=True, shader=fs_default, uv_panning=[1.0,1.0], uv_rotation=1.0, **kwargs):
        if texture == None:
            if source == None:
                return
            else:
...
class ShaderTreeApp(App):
    def build(self):
        ...
        texture = texture=Image(source='star.png').texture
