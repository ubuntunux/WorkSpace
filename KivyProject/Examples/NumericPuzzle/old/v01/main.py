...
class NumButton(Button):
	...
	def hide(self):
		if self.isShow:
			...
			self.background_color= (self.background_color[0], self.background_color[1], self.background_color[2], 0)
			
	def show(self):
		if self.isShow == False:
			self.isShow = True
			self.color= (self.color[0], self.color[1], self.color[2], 1)
			self.background_color= (self.background_color[0], self.background_color[1], self.background_color[2], 1)
	...
	def buildNumeric(self, cols=0, rows=0):
		...
		for i in range(cols):
			for j in range(rows):
				...
				btn.bind(on_press=self.pressButton)
