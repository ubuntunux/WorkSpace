...
class Sprite(Widget, object):
  ...
  def __init__(self, pos=cXY, size=(100.0, 100.0), **kargs):
    ...
    self.dragable = True
    self.throwable = True
    self.func_touch_down = None
    self.func_touch_move = None
    self.func_touch_up = None
      ...
      if self.touchable and not self.isAttached:
        if touch.pos[0] > self.boxPos.x and touch.pos[1] > self.boxPos.y and \
          touch.pos[0] < self.boxPos.x + self.realSize[0] and touch.pos[1] < self.boxPos.y + self.realSize[1]:
            ...
            if self.func_touch_down:
              self.func_touch_down(touch)
  ...
  def on_touch_move(self, touch):
    if touch.grab_current is self:
      ...
      if self.func_touch_move:
        self.func_touch_move(touch)
    else:
      self.isTouched = False
  ...
  def on_touch_up(self, touch):
    if touch.grab_current is self:
      ...
      touch.ungrab(self)
      if self.func_touch_up:
        self.func_touch_up(touch)
  ...
  def setOnGround(self, onGround):
    ...
    if onGround:
      self.setVelocity(self.vel[0], 0.0)
  ...
  def update(self, fFrameTime):
    ...
    if self.updateTranslate and not self.isAttached:
      # set gravity
      if self.gravity != 0 and not self.onGround:
